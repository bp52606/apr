...
#klasa koja implementira Eulerov algoritam
class Euler(Algorithm):
    x: Matrica
    t: float
    f: Function
    T: float
    ...
    def algorithm(self, f: Function, implicit=False, xprev=False, draw=True, four=False, step=30, fname="", prvi=False):
        ...
        for i in x_axis:
            ...
            if four:
                self.r[0, 0] = i                                #racunanje novih vrijednosti r(t)
                self.r[1, 0] = i
            xcurr = ((U + self.A * self.T) * xkeep) + (self.T * self.B) * self.r        #racunanje nove vrijednosti tocke
            file.write(str(xcurr) + "\n")
            if prvi:
                sum += calculateX(self.x, xcurr, i)             #racunanje razlike izmedu tocaka za prvi zadatak
